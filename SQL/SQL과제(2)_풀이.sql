...
--(1) 도서번호가1인도서의이름 : book 테이블
select bookname
from book
where bookid = 1
;

--(2) 가격이20,000원이상인도서의이름 : book 테이블
select bookname
from book
where price >= 20000
;

--(3) 박지성의총구매액(박지성의고객번호는1번으로놓고작성) : orders 테이블
...
--(1) 마당서점도서의총개수 :book테이블의 총 row의 개수 
